...
func (a *App) GetDatabaseStatus() DatabaseStatus {
	...
	status.Message = fmt.Sprintf("已执行 %d 个迁移", status.MigrationCount)
	return status
}

// InitDatabase 初始化数据库（创建数据库 + Prisma 迁移 + Seed）
// 仅用于首次部署空数据库，失败时不写入「已初始化」状态
func (a *App) InitDatabase() string {
	webDir := a.findWebDir()
	if webDir == "" {
	...
	// 第三步：执行 Seed
	runtime.EventsEmit(a.ctx, "log", fmt.Sprintf("[%s] 执行数据种子 (bun x prisma db seed)...\n", timestamp()))
	output, err = a.runPrismaCommand(webDir, []string{"db", "seed"})
	runtime.EventsEmit(a.ctx, "log", output+"\n")
	if err != nil {
		// Seed 失败，不写入「已初始化」状态
		return "Seed 执行失败: " + err.Error() + "（表结构已创建但预设数据未写入，请检查日志后重试）"
	}
...
func (a *App) runPrismaCommand(webDir string, args []string) (string, error) {
	...
	// 将 Bun 目录添加到 PATH，确保 seed 命令中的 `bun` 可被 Prisma CLI 找到
	cmd.Env = append(os.Environ(),
		fmt.Sprintf("PATH=%s;%s", bunDir, os.Getenv("PATH")),
		fmt.Sprintf("DATABASE_URL=%s", dbURL),
		fmt.Sprintf("DATABASE_HOST=%s", a.config.Database.Host),
		fmt.Sprintf("DATABASE_PORT=%d", a.config.Database.Port),
		fmt.Sprintf("DATABASE_USER=%s", a.config.Database.Username),
		fmt.Sprintf("DATABASE_PASSWORD=%s", password),
		fmt.Sprintf("DATABASE_NAME=%s", a.config.Database.DBName),
		fmt.Sprintf("JWT_SECRET=%s", a.config.JWTSecret),
	)
...
func (a *App) StartService() string {
	...
	cmd.Env = append(os.Environ(),
		fmt.Sprintf("PATH=%s;%s", bunDir, os.Getenv("PATH")),
		fmt.Sprintf("PORT=%d", a.config.WebPort),
		fmt.Sprintf("DATABASE_URL=%s", dbURL),
		fmt.Sprintf("DATABASE_HOST=%s", a.config.Database.Host),
		fmt.Sprintf("DATABASE_PORT=%d", a.config.Database.Port),
		fmt.Sprintf("DATABASE_USER=%s", a.config.Database.Username),
